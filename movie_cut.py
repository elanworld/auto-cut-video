...
import cv2
import numpy as np
from CompareFrame import CompareFrame
...
class CutMovie(CompareFrame):
    ...
    def saveOrPassAndmkdir(self,frames=None,rate=None,startFrame=None, stopFrame=None):
    ...
    def save_video(self,frames, rate , size):
        if not self.saveOrPassAndmkdir(frames=frames,rate=rate,startFrame=None, stopFrame=None):
            return
        self.video_splt_time.append(len(frames))
        save_filename = os.path.join(self.videoPath , self.name + '_' + str(self.saveNumber) + ".avi")
        self.save_filename_list.append(save_filename)
        self.saveNumber += 1
        #print(save_filename)
        writer = cv2.VideoWriter()
        writer.open(save_filename, int(1145656920), rate, size, True)  # 参数2为avi格式的forucc，使用别的格式会导致转换出错
        for frame in frames:
            writer.write(frame)
        return True

    def save_video_with_ffmpeg(self,filename, startFrame=0, stopFrame=1, rate=24):
        if self.saveOrPassAndmkdir(startFrame=startFrame, stopFrame=stopFrame,rate=rate):
            return
        self.video_splt_time.append([startFrame,stopFrame])
        ffmpegPath = "ffmpeg"
        save_filename = os.path.join( self.videoPath , self.name + '_' + str(self.saveNumber) + ".ts")
        self.save_filename_list.append(save_filename)
        self.saveNumber += 1
        #print(save_filename)
        cmd = [ffmpegPath, "-y", "-ss", str(startFrame / float(rate) ), "-t", str( ( stopFrame - startFrame ) / float(rate) )
                                , "-i", filename,"-acodec","copy" ,"-vcodec","copy", "-vbsf", "h264_mp4toannexb",
                            save_filename,]
        cmd1 = [ffmpegPath, "-y", "-ss", str(startFrame / float(rate) ), "-t", str( ( stopFrame - startFrame ) / float(rate) )
                                , "-i", filename,"-an" ,"-vcodec","copy", "-vbsf", "h264_mp4toannexb",
                            save_filename,]
            ...
            while currentFrame <= frameToEnd:
                ...
                if ((not isSimilar) or (currentFrame >= frameToEnd)):
                    if cvSave:
                        ...
                        self.save_video_with_ffmpeg(filename, frameToStart, currentFrame, rate)
    ...
    def audioDigital(self,audio):
        ...
        while True:
            ...
            time_cut_list = np.split(time_select, np.where(np.diff(time_select) != split_time)[0] + 1)
    ...
    def video_concat(self,dir):
        ...
        cmd = '''{} -y -f concat -safe 0 -i "{}" -i "{}" -vcodec copy -filter_complex "[0:a]aformat=sample_fmts=fltp:sample_rates=44100:channel_layouts=stereo,volume=0.2[a0]; [1:a]aformat=sample_fmts=fltp:sample_rates=44100:channel_layouts=stereo,volume=0.5[a1]; [a0][a1]amerge=inputs=2[aout]" -map 0:v  -map "[aout]" -shortest "{}"'''.format(ffmpeg,self.videoInfo,self.bgm,self.videoOutput)
        cmd = '''{} -y -f concat -safe 0 -i "{}" -i "{}" -vcodec copy -c:a copy -shortest "{}"'''.format(ffmpeg,self.videoInfo,self.bgm,self.videoOutput)
        print("cmd:",cmd)
        os.chdir(dir)
        os.system(cmd)
